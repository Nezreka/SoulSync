...
from core.metadata import registry as _metadata_registry
...
METADATA_SOURCE_PRIORITY = _metadata_registry.METADATA_SOURCE_PRIORITY
...
def get_primary_source() -> str:
    ...
    return _metadata_registry.get_primary_source(spotify_client_factory=SpotifyClient)
...
def get_source_priority(preferred_source: str):
    ...
    return _metadata_registry.get_source_priority(preferred_source)


def get_spotify_client():
    """Get cached Spotify client."""
    if SpotifyClient is None:
        return _metadata_registry.get_registered_runtime_client('spotify')
    return _metadata_registry.get_spotify_client(client_factory=SpotifyClient)
...
def get_deezer_client():
    ...
    """
    return _metadata_registry.get_deezer_client()


def get_itunes_client():
    """Get cached iTunes client."""
    return _metadata_registry.get_itunes_client(client_factory=iTunesClient)
...
def get_discogs_client(token: Optional[str] = None):
    ...
    return _metadata_registry.get_discogs_client(token=token, client_factory=DiscogsClient)


def is_hydrabase_enabled() -> bool:
    """Return True when Hydrabase is connected and allowed for metadata use."""
    enabled = _metadata_registry.is_hydrabase_enabled()
    if enabled:
        return True

    # Compatibility fallback for legacy callers that still bootstrap
    # Hydrabase through `web_server` without registering runtime clients.
    try:
        import importlib
...
def get_hydrabase_client(allow_fallback: bool = True, require_enabled: bool = True):
    ...
    """
    client = _metadata_registry.get_hydrabase_client(allow_fallback=False, require_enabled=require_enabled)
...
def clear_cached_metadata_clients():
    ...
    with _client_cache_lock:
        _client_cache.clear()
    _metadata_registry.clear_cached_metadata_clients()
