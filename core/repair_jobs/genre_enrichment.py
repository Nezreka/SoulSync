    ...
    def scan(self, context: JobContext) -> JobResult:
        ...
        try:
            if settings['include_artists']:
                ...
                cur = conn.cursor(); cur.execute("SELECT * FROM artists")
                columns = [x[0] for x in cur.description]
                rows += [('artist', dict(zip(columns, r))) for r in cur.fetchall()]
            if settings['include_albums']:
                cur = conn.cursor(); cur.execute("SELECT * FROM albums")
                columns = [x[0] for x in cur.description]
                rows += [('album', dict(zip(columns, r))) for r in cur.fetchall()]
        finally: conn.close()
        if context.update_progress: context.update_progress(0, len(rows))
        ...
        for index, (kind, row) in enumerate(rows):
            ...
            local = collect_local_candidates(row, kind)
            ...
            if context.update_progress and (index + 1) % 100 == 0: context.update_progress(index + 1, len(rows))
        if context.update_progress: context.update_progress(len(rows), len(rows))
