...
const BASE = {
  row: {
    search_attempts: 9,
    last_search_at: '2026-09-02 12:00',
...
describe('the drawer', () => {
        ...
        search_snapshot: {
          ...
          sources: {
            torrent: {
              ...
              samples: [
                {
                  title: 'Big Brother US S28E25 1080p',
                  accepted: false,
                  rejected: 'Wrong episode',
                },
                {
                  title: 'Big Brother US S28E27 2160p',
                  accepted: false,
                  rejected: '4K not enabled',
                },
              ],
            },
          },
        ...
        search_snapshot: {
          ...
          sources: {
            torrent: {
              ...
              results: 1,
              accepted: 1,
              samples: [{ title: 'Good one', accepted: true, rejected: null }],
            },
          },
        },
      },
  ...
  it('says a source could not search rather than implying it found nothing', () => {
    ...
    expect(host.querySelector('.vwsh-diag-src')?.textContent).toContain(
      'could not search: slskd is unreachable',
    );
  });
  ...
  it('explains an empty receipt list rather than showing a blank block', () => {
    ...
    const host = render({
      ...
      row: {
        ...
        search_snapshot: {
          chain: ['torrent'],
          sources: { torrent: { ran: true, results: 15, accepted: 0 } },
        },
      },
    });
    expect(host.querySelector('.vwsh-diag-rel--none')?.textContent).toContain(
      'not been searched since receipts were added',
    );
  });
