...
export function ReassignModal({
  ...
  const [sources, setSources] = useState<{ id: string; label: string; active: boolean }[] | null>(
    null,
  );
  ...
  useEffect(() => {
    let alive = true;
    void fetchReidentifySources().then((rows) => {
      if (!alive) return;
      // The endpoint's field is `source`, not `id`, and it flags the active
      // one — pre-select THAT rather than whichever happens to be first.
      const list = (rows || []).map((r) => ({
        id: String(r.source),
        label: String(r.label ?? r.source),
