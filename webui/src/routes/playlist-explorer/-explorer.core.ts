...
/** _explorerLoadPlaylists (:74) — the tab labels; anything unlisted is Title-cased. */
...
/**
 * _explorerLoadPlaylists (:50) — group by lowercased source, first-seen order. The tab strip hides
...
export interface ExplorerCardView {
  ...
  /** The meta line's tail and the class the vanilla wrapped it in (explorerRenderPickerCards :134). */
  metaText: string;
  metaClass: 'explorer-picker-discovered' | 'explorer-picker-not-ready' | null;
  /** The second meta row, empty when neither counter applies (explorerRenderPickerCards :146). */
  statusParts: { className: string; text: string }[];
  showDiscoverButton: boolean;
}

/**
 * explorerRenderPickerCards (:102) — the whole card derivation. The badge ladder is a precedence
...
/**
 * _explorerRenderArtistNode (:428) — the branch/node id stem. Collision-prone by construction (every
...
/** _explorerGetOrCreateRow (:407) — row N holds N+2 artists, which is what gives the tree its taper. */
export function explorerRowCapacity(rowIndex: number): number {
  return rowIndex + 2;
}

/**
 * _explorerGetOrCreateRow (:402) — the streamed rows, resolved up front. Artists arrive one NDJSON
...
/** explorerToggleArtist (:470) — real Spotify id when there is one, else a positional stand-in. */
...
/**
 * explorerToggleAlbum (:561) — the tracklist fetch only fires for real ids. The vanilla's test is
...
/** explorerToggleArtist (:475), explorerAddToWishlist (:667) — the type pill. */
...
/** _formatDuration (:542) — blank rather than "0:00" when the API omits the duration. */
...
/**
 * explorerBuildTree (:320) — the build progress bar and its caption.
 *
 * One deliberate divergence: the vanilla divided by the streamed total with no
 * guard, so a playlist reporting 0 artists wrote `width: NaN%` into the bar.
 * Zero here instead.
 */
...
/** _explorerUpdateCount (:621) — "0 albums selected". */
export function explorerSelectionLabel(count: number): string {
  return `${count} album${count !== 1 ? 's' : ''} selected`;
}

/**
 * explorerSelectAll (:592) — Select All takes only real ids and skips what is already owned,
...
/** _explorerRefreshArtistIndicators (:632) — the `.has-selection` ring on a collapsed artist node. */
...
/**
 * explorerAddToWishlist (:643) — the wishlist modal's grouping. Artists with nothing selected drop
...
/** explorerAddToWishlist (:659) — the modal's hero subtitle counters. */
...
/** _explorerDrawCurve (:991) — the cubic that links a parent's bottom to a child's top. */
...
/** _explorerDrawCurve (:989) — each tier gets its own gradient and weight. */
...
/** _explorerSizeSvg (:961) — the SVG is sized in unscaled tree space, plus 40px of slack. */
...
/**
 * _explorerGetPos (:974) — getBoundingClientRect reports SCALED pixels, but the SVG lives
...
/**
 * explorerFitToView (:1043) — leaves 20px of margin on each side and refuses
 * to zoom past 1.5, so a two-node tree doesn't balloon.
 *
 * Takes the tree's UNSCALED size. The vanilla reset the transform to scale(1)
 * before measuring; that was redundant, because scrollWidth/scrollHeight are
 * layout values a CSS transform does not affect — see the caller in
 * -explorer.interactions.ts, which measures without touching the DOM.
 */
...
/** explorerFitToView (:1065) — centre the scaled tree horizontally after a fit. */
