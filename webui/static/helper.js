...
const WHATS_NEW = {
    ...
    '3.2.3': [
        { date: 'August 2026 · 3.2.3' },
        { title: 'The sync page is a playlist library now', desc: 'search it, filter by scheduled or unscheduled, and the three tabs finally have names that say what is in them. every card does its own work: schedule a playlist from its own card without opening a modal, pick quality, sync now, all from the popover. the popovers stay on screen instead of running off the bottom and they work on touch. the cards also stopped guessing how much of a playlist you own, the matcher records it now, so the number comes from the same place the sync does.', page: 'sync' },
        { title: 'Downloads say what they are actually doing', desc: 'no more sitting on "Loading...". the cards, the modal and album releases narrate the live search and download: what it is searching, what it found, what it is pulling. pasting a deezer link does the same instead of going quiet. (#1156)', page: 'active-downloads' },
        { title: 'Soulseek stopped ignoring your concurrency limit', desc: 'the limit was enforced per batch and so was the lock guarding it, so three batches each happily ran their "one" download at the same time. it is a global cap now. a batch that would exceed it holds and says so, and whoever frees a slot nudges the batches that are waiting, so the cap cannot turn into a stall. (#1166)', page: 'active-downloads' },
        { title: 'Approving a quarantined file makes it stay approved', desc: 'people were approving the same songs five times over. the list that stops a known-bad upload being picked again had no storage of its own, it was just the files sitting in the quarantine folder, so approving or clearing the list erased the record of what had already failed and the same broken file won again. that verdict is in the database now. the count at the top updates on its own instead of only when you click into the tab, and the dashboard has a Review button showing how many files are waiting.', page: 'active-downloads' },
        { title: 'YouTube Music at the quality you pay for', desc: 'searches the youtube music catalog with cookies.txt auth, fetches premium itags, and ranks original opus and aac against your quality profile instead of guessing. mixed pools get scored per source, so a youtube result and a soulseek result are judged on the same terms. thanks lucasssvaz', page: 'settings' },
        { title: 'The expired download cleaner keeps music people care about', desc: 'it could delete files someone had favourited. it reads favourites, ratings and playlists off plex and jellyfin now and spares anything anyone marked. it also refuses to delete on a half-finished sweep, because a media server being briefly down used to look exactly like nobody liking anything, which is precisely when it starts deleting.', page: 'tools' },
        { title: 'Quality profiles save what you sent', desc: 'eight settings could never be changed through the api, and the obvious fix turned out worse: reordering your format ladder would silently switch off lossy copy, acoustid strictness, deep verify and replace lower quality. naming a key saves it now, including turning it off, and leaving it out leaves it alone. editing one of those while previewing a different profile finally tells you it will not stick. (#1103)', page: 'settings' },
        { title: 'Earlier versions', desc: '3.2.2 added a way to reassign an album to the right artist and fixed folders that differ only in capitalization. 3.2.1 made hand-picked artwork survive a sync and gave the artwork cache a ceiling. 3.2.0 moved sixteen music pages to React. 3.1.9 added the Arcade.' },
    ],
};
...
const VERSION_MODAL_SECTIONS = [
    {
        title: "3.2.3: the sync page grows up, and downloads stop being silent",
        description: "the biggest release since the react move, and most of it is things that were quietly wrong rather than missing. the sync page became a library you can actually search and schedule from. downloads narrate what they are doing instead of saying Loading. and a run of reported bugs got their real cause: a concurrency limit that was never global, a request endpoint that returned 500 on success, and a quarantine list that forgot what it had already rejected.",
        features: [
            "the sync page is a playlist library. search it, filter by scheduled or unscheduled, and the three tabs have names now. schedule a playlist from its own card without opening a modal, pick quality, sync now, all from the popover, which stays on screen and works on touch. the cards stopped guessing how much of a playlist you own too, the matcher records it, so the number comes from the same place the sync does. mobile got a real pass: the filter chips scroll, the page stopped being 793px wide on a 390px screen, and the buttons stopped landing on top of the playlist name",
            "downloads say what they are doing (#1156). the cards, the modal and album releases narrate the live search and download instead of sitting on Loading. same for pasting a deezer link",
            "soulseek finally honours your concurrency limit (#1166). it was enforced per batch and so was the lock guarding it, so three batches each ran their \"one\" download at once. the cap is global now, a batch that would exceed it holds and says so, and whoever frees a slot nudges whoever is waiting so the cap cannot become a stall",
            "approving a quarantined file makes it stay approved. the list that stops a known-bad upload being re-picked had no storage of its own, it was the files sitting in the quarantine folder, so approving or clearing the list erased the record of what had failed and the picker chose the same broken file again. that verdict lives in the database now, the count at the top updates on its own, and the dashboard has a Review button showing what is waiting",
            "youtube music at the quality you pay for, thanks lucasssvaz (#1154). catalog search with cookies.txt auth, premium itags, and original opus and aac ranked against your quality profile rather than guessed. mixed pools are scored per source so a youtube result and a soulseek result are judged on the same terms",
            "the expired download cleaner keeps music people care about. it reads favourites, ratings and playlists off plex and jellyfin and spares anything anyone marked, and it refuses to delete on a half-finished sweep, because a server being briefly down used to look identical to nobody liking anything",
            "quality profiles save what you sent (#1103). eight settings could never change through the api, and writing all of them instead turned out worse: reordering your format ladder silently switched off lossy copy, acoustid strictness, deep verify and replace lower quality. presence decides now, and editing one of those while previewing another profile tells you it will not stick",
            "POST /api/v1/request answered 500 on every success and could never reach a terminal state (#1168). it answers 202, and transfers settle from one shared sweep instead of a thread each",
            "wishlist and matching, thanks nstrelow: wing it can search unverified guesses (#1147), musicbrainz artist aliases resolve before giving up on a track (#1161), and the download query ladder got a version-stripped last resort (#1163)",
            "reported fixes: the sidebar reclaims its space when collapsed (#1155), manual matches stick and jellyfin remove/replace works (#1159), modal thumbs go through the proxy and title-only matches stop reporting 100% (#1164), the soulseek filter honours per-item quality profiles (#1150), navidrome paths resolve when the filename is synthesized (#1127), and file_path corruption during library scans is fixed thanks to shkarlsson (#1158)",
        ],
        usage_note: "nothing to turn on. the sync changes are on the sync page, the Review button is on the dashboard library strip, and the download narration shows on the downloads page and in the download modal. the expired download cleaner and its curation reader are opt-in under Tools.",
    },
    {
        title: "Earlier in 3.2.2: put an album back where it belongs",
        description: "one new thing and five fixes, four of them straight from reports.",
        features: [
            "reassign an album to a different artist, in the album action row next to Reorganize and Redownload, for when a featured artist got picked up as the album artist. each track goes back through the import pipeline so tags, folders and database rows come from the same code that handles a fresh download",
            "a slow prowlarr indexer no longer throws away everyone else's results (#1151), and folders that differ only in capitalization stopped making two albums (#1091)",
            "Quality Check upgrades failed every time with \"no matched track in finding\", and \"replace the original file\" did nothing on a manual import. both fixed",
        ],
    },
    {
