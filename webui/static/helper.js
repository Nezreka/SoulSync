...
const WHATS_NEW = {
    ...
    '3.3.0': [
        { date: 'August 2026 \u00b7 3.3.0' },
        { title: 'Discover is a discovery surface now, not a wall', desc: 'the page is built around a hero, a priority queue and grouped recommendation zones instead of one long scroll of everything at once. the discovery data and the actions behind it are unchanged, this is about what you see first and what you can act on. the empty state says something useful now, mobile got real density work rather than the desktop layout squeezed, and the genre explorer and build-playlist flows read as parts of one page.', page: 'discover' },
        { title: 'Last.fm listening history feeds your stats', desc: 'an importer that uses the last.fm settings you already have, runs through automation like everything else, and works the username out itself when auth can. the careful part is large backfills: the checkpoint state is kept separate from the incremental cursor, an interrupted page resumes instead of restarting, and a duplicate page is no longer mistaken for "we reached the end", which is how a half-finished import used to quietly mark itself complete.', page: 'stats' },
        { title: 'Stats charts open the tracks behind them', desc: 'clicking into When You Listen or Listening Activity now opens the actual tracks behind that bar, with play actions, artist links, artwork, timestamps and where the play came from. backed by real listening-event queries rather than re-deriving it from the chart.', page: 'stats' },
        { title: 'The watchlist got a proper pass', desc: 'the music watchlist experience is upgraded across the board, and the artist dock got a mobile pass so it works as a phone control instead of a desktop one at phone width.', page: 'watchlist' },
        { title: 'MusicBrainz stopped timing out on slow connections', desc: 'every musicbrainz call carried its own hardcoded 10 second read timeout, which is fine on a fast link and not fine on shared egress. they share one helper with a 30 second timeout now and retry read timeouts, connection errors and 429/503/504 instead of giving up on the first one. the retry keeps the one-request-per-second pacing, which is the easy bit to get wrong. both are settings. (#1177)' },
        { title: 'MusicMap was being sent junk artist names', desc: 'a tag polluted with commas, semicolons or a "feat" clause was sent as one encoded joined string that matched nothing. names get cleaned before the url is built, and placeholder names are skipped before the request happens at all, so your log stops filling with upstream 404s. (#1178)' },
        { title: 'Cleanup could still make a Navidrome path look deleted', desc: 'the last of the delete surfaces that could report success on a file it never touched. the expired download cleaner resolves paths with the same transfer-folder context as the other repair jobs, and keeps the origin and history rows when a navidrome virtual path cannot be mapped. repair actions now say out loud when only the database row went. (#1127)', page: 'tools' },
        { title: 'Large Deezer playlists survive a reverse proxy', desc: 'a big deezer playlist used to depend on one long request staying open while track numbers resolved, which is exactly the request a proxy kills. loads run as backend jobs now and the page polls until the payload is ready, with the progress frames still narrating the slow phase. a proxy HTML response also becomes a real error instead of an "Unexpected token" toast.', page: 'sync' },
        { title: 'Artwork loads without the service worker', desc: 'cover art archive and archive.org images go through the same-origin image proxy when the page is not controlled by the service worker, and keep the direct path when it is. applied to artist detail, the discover album shelves and album import art. (#1179)' },
        { title: 'Earlier versions', desc: '3.2.4 paced prowlarr searches so indexers stop getting hammered, and stopped oversized cover art taking a file\u2019s tags down with it. 3.2.3 rebuilt the sync page into a playlist library and made downloads narrate what they are doing. 3.2.0 moved sixteen music pages to React.' },
    ],
};
...
const VERSION_MODAL_SECTIONS = [
    {
        title: "3.3.0: Discover rebuilt, and your listening history arrives",
        description: "a feature release. discover is a discovery surface rather than a wall of everything, last.fm listening history feeds the stats page, and the watchlist got a proper pass. plus a run of reported bugs, most of them the kind that fail quietly rather than loudly.",
        features: [
            "discover is built around a hero, a priority queue and grouped recommendation zones instead of one long scroll. the discovery data and the actions behind it are unchanged \u2014 this is about what you see first and what you can act on. the empty hero state says something useful now, mobile got real density work rather than the desktop layout squeezed into a phone, and the genre explorer and build-playlist flows read as parts of one page instead of three ideas sharing a route",
            "last.fm listening history has an importer. it uses the last.fm settings you already have, runs through automation like everything else, reports progress through notifications, and does not need a separate username when auth can resolve it. the careful part is a large backfill: checkpoint state is kept separate from the incremental cursor, an interrupted page resumes instead of restarting, and a duplicate page is no longer mistaken for reaching the end \u2014 which is how a partial import used to quietly mark itself complete",
            "stats charts open the tracks behind them. clicking into When You Listen or Listening Activity gives you the actual plays: play actions, artist links, artwork, timestamps and the source. backed by real listening-event queries rather than re-deriving from the chart",
            "the music watchlist is upgraded across the board, and the artist dock got a mobile pass so it behaves like a phone control instead of a desktop one at phone width",
            "musicbrainz stopped timing out on slow connections. every call had its own hardcoded 10 second read timeout, which is fine on a fast link and not on shared egress. one helper with 30 seconds now, retrying read timeouts, connection errors and 429/503/504 \u2014 while preserving the process-wide one-request-per-second pacing, which a naive retry loop gets wrong by waiting politely between calls and then hammering the retries. both tunable in settings (#1177)",
            "musicmap was being handed junk names: a tag polluted with commas, semicolons or a \"feat\" clause went out as one encoded joined string matching nothing. names are cleaned before the url is built, placeholders are skipped before the request happens at all, and the legacy watchlist scanner path got the same guard so background and live lookups agree (#1178)",
            "cleanup could still make a navidrome path look deleted \u2014 the last of the surfaces that could report success on a file it never touched. the expired cleaner resolves paths with the same context as the other repair jobs and keeps history rows when a virtual path cannot be mapped, album delete returns real file errors instead of a bare count, and repair actions say when only the database row went. fuzzy matching stays out of destructive paths on purpose (#1127)",
            "large deezer playlists survive a reverse proxy: loads run as backend jobs handing back a job id rather than one long-held request, with the page polling until the payload is ready. a proxy HTML response now becomes a readable error instead of an \"Unexpected token\" toast",
            "artwork loads when the service worker is not in control, routed through the same-origin image proxy and keeping the direct path when it is (#1179). and the soulseek gate no longer blocks a hybrid fallback from running",
        ],
        usage_note: "the last.fm importer lives on Settings \u2192 Connections with your existing Last.fm credentials, and runs as an automation once enabled. the musicbrainz timeout and retry count are settings too, worth raising if you run behind slow shared egress.",
    },
    {
