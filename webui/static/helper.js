...
const WHATS_NEW = {
    ...
    '3.1.8': [
        ...
        { title: 'Channels + threads', desc: 'the room now has channels (#general, #off-topic, #help, #bugs, #ideas) with unread badges — they\'re tags carried in the SoulSync envelope, so untagged and vanilla-client messages always land in #general and nothing is ever hidden. 🧵 any message to start a thread; a channel\'s threads hang beneath it in the sidebar and open in their own view. channels are for the SoulSync room only — other Soulseek rooms stay plain unfiltered chat.' },
        { title: 'Mention ping + now-playing', desc: 'an optional sound when someone @-mentions you or replies to you — off by default, throttled, and silent while a room\'s history is loading. and an opt-in activity line that shows the room what you\'re playing in SoulSync. both live in the chat settings cog.' },
        { title: 'Auto-DJ is a real radio now', desc: 'when the jukebox queue runs dry, Auto-DJ builds an actual radio from what the room just played — Last.fm similar-tracks, then similar-artists, then your own library\'s similar-artist graph — instead of requeueing the same song. if the last thing played was a mix rather than a single track it follows the genre instead. it tells you why it\'s idle, and the room elects one DJ so two people enabling it can\'t double-queue. jukebox playback is also timed off your own clock now, so a wrong system time no longer starts every track at the wrong spot.' },
        ...
        { title: 'Discover loads faster', desc: 'the page fired ~20 section requests at once and choked the backend; it now loads the top of the page first and streams the rest in, so it\'s usable in a couple seconds instead of half a minute. genre explorer pills are cached too, so reopening one is instant instead of another ~30s scan. also fixed the sidebar header showing through the artist map.' },
