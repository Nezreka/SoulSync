...
function startSyncPolling(playlistId) {
    ...
    activeSyncPollers[playlistId] = setInterval(async () => {
        ...
        try {
            ...
            // `playlist` is not in this function's scope — reading it here threw a
            // ReferenceError on every poll tick (optional chaining does not guard an
            // undeclared identifier). The poll response carries the name when the
            // server knows it, and `...state` below supplies it.
            const syncFrame = { playlist_id: playlistId, playlist_name: '', ...state };
